...
export default function BasicInfoForm() {
  ...
  const saveFormData = () => {
    if (
      ...
      const res = update_resume(postData,URL.UPDATE_BASIC_INFO);
    ...
    (async () => {
      try {
        ...
        if(Cookie.get("resume_id")){
          ...
          const responseBasicInfo = await fetch(URL.GET_RESUME, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ resume_id: resume_id }),
          });
          const resume = await responseBasicInfo.json();
          const basicInfo = resume.resume.basic_info[0];
          setfirstName(basicInfo.first_name);
          setlastName(basicInfo.last_name);
          setProfession(resume.resume.resume[0].profession_title);
          setEmail(basicInfo.email);
          setPhone(basicInfo.phone);
          setAddress(basicInfo.address);
          setpostalCode(basicInfo.zip);
          setCountry(basicInfo.country);
        }
        setisLoaderOn(false);
      } catch (e) {
        console.log(e);
