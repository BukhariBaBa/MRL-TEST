...
export default function WorkHistoryContentForm() {
  ...
  const addContent = (e) => {
    ...
    if (e.target.classList.contains(Style.active)) {
      ...
      let new_str = dataText + "<li>" + new_strr + "</li>";
      Cookie.set("selected_content", new_str);
    }
  ...
  const saveData = () => {
    ...
    router.push(`/builder/edit/${PAGE_ROUTE.WORK_HISTORY_SUMMERY}`);
  };
  useEffect(() => {
    Cookie.set("selected_content", "");
    if(Cookie.get('work_history_content')){
      // console.log("content:", JSON.parse(Cookie.get('work_history_content')));
